...
Script components are added to entities and Scriptlets are added to the
Script.  Scriptlets can implement arbitrary run() methods, which are
called every tick.
...
class Script(ecs.Component):
    ...
    """
    Due to how ECS is implemented, you need to add a Script to an Entity
    and then add Scriptlets to it.  Allows Entities to run arbitrary
    code.
    """
...
class ScriptSystem(ecs.System):
    ...
    def __init__(self, world, master):
        """
        `world` is an ECS World, self-explanatory.  `master` is the
        master object, which owns the `world` and generally is running
        the show.  In this case, it'd probably be a State (like
        GameScene).
        """
